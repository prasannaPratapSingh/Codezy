...
const ProblemPage = () => {
    ...
    useEffect(() => {
        const fetchProblem = async () => {
            setLoading(true);
            try {
                const response = await axiosClient.get(`/contest/getContest/${problemId}`);
    ...
    const handleRun = async () => {
        ...
        try {
            const response = await axiosClient.post(`/contest/run/${problemId}`, {
                code,
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            const response = await axiosClient.post(`/contest/submit/${problemId}`, {
                code: code,
                language: selectedLanguage
            });
            setSubmitResult(response.data);
            setLoading(false);
            toast("Code Submitted Successlly, wait for the contest to get over for RESULTS",{
                icon:'👩🏻‍💻'
            })
            setTimeout(() => {
                navigate('/contest')
            ...
            if (error.response.data === "Already participated in the Contest, wait for the RESULT!") {
                toast.success(error.response.data);
                setTimeout(() => {
                    navigate('/contest');
                }, 2000);
            }
            if (error.response.data === "Time's Up!") {
                toast.success(error.response.data);
                setTimeout(() => {
                    navigate('/contest');
                }, 2000);
